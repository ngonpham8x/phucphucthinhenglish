...
export const DashboardView: React.FC<DashboardViewProps> = ({ students, teachers, classes, rooms, receipts, programs, currentUser, onNavigateTab }) => {
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      {showDebt && <section className="grid grid-cols-1 gap-4 sm:grid-cols-4"><div className="rounded-xl border border-slate-200 bg-white p-4 shadow-2xs"><p className="text-xs font-semibold text-slate-500">Tổng phải thu</p><p className="mt-1 text-xl font-extrabold">{currency(totalCollected + totalDebt)}</p></div><div className="rounded-xl border border-slate-200 bg-white p-4 shadow-2xs"><p className="text-xs font-semibold text-slate-500">Đã thu</p><p className="mt-1 text-xl font-extrabold text-emerald-700">{currency(totalCollected)}</p></div><Card label="Nợ học phí tháng" value={currency(totalDebtBreakdown.monthly)} icon={AlertTriangle} color="bg-rose-100 text-rose-600" detail="monthly-debt" subtitle="Bấm để xem lớp và học viên" /><Card label="Nợ học phí khóa" value={currency(totalDebtBreakdown.course)} icon={AlertTriangle} color="bg-rose-100 text-rose-600" detail="course-debt" subtitle="Bấm để xem lớp và học viên" /></section>}
